...
pipeline = [
    {
        "$match": {
            ...
            "Sodium, Na-MG": {"$exists": True, "$ne": None, "$gte": 0},
            "Energy-KCAL": {"$gt": 0},
            "serving_size": {"$gt": 0}
        }
    },
    {
        "$project": {
            ...
            "fat": {"$ifNull": ["$Total lipid (fat)-G", 0]},
            "carbs": {"$ifNull": ["$Carbohydrate, by difference-G", 0]},
            "potassium": {"$ifNull": ["$Potassium, K-MG", 0]},
            "fiber": {"$ifNull": ["$Fiber, total dietary-G", 0]},
            "saturated": {"$ifNull": ["$Fatty acids, total saturated-G", 0]},
            "sodium_per_100kcal": {
                "$multiply": [
                    {"$divide": ["$Sodium, Na-MG", "$Energy-KCAL"]},
                    100
                ]
            },
            "sodium_per_serving": {
            ...
            "sodium_to_potassium_ratio": {
                "$cond": [
                    {"$gt": [{"$ifNull": ["$Potassium, K-MG", 0]}, 0]},
                    {"$divide": ["$Sodium, Na-MG", "$Potassium, K-MG"]},
                    None
                ]
            },
            "sodium_to_protein_ratio": {
                "$cond": [
                    {"$gt": [{"$ifNull": ["$Protein-G", 0]}, 0]},
                    {"$divide": ["$Sodium, Na-MG", "$Protein-G"]},
                    None
                ]
            },
