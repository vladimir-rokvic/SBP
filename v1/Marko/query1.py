...
pipeline = [
    {
        "$match": {
            ...
            "Protein-G": {"$exists": True, "$ne": None},
            "Total lipid (fat)-G": {"$exists": True, "$ne": None},
    ...
    },
    {
        "$project": {
            ...
            "monounsaturated": {"$toDouble": "$Fatty acids, total monounsaturated-G"},
            "polyunsaturated": {"$toDouble": "$Fatty acids, total polyunsaturated-G"},
    ...
    },
    {
        "$project": {
            ...
            "unsaturated_fat": {"$add": ["$monounsaturated", "$polyunsaturated"]},
            "sodium_per_serving": {
                "$cond": [
                    {"$gt": ["$serving_size", 0]},
            ...
            "calories_from_protein": {"$multiply": ["$protein", 4]},
            "calories_from_fat": {"$multiply": ["$fat", 9]},
            "calories_from_carbs": {"$multiply": ["$carbs", 4]}
        }
    },
    {
        "$group": {
            ...
            "avg_monounsaturated": {"$avg": "$monounsaturated"},
            "avg_polyunsaturated": {"$avg": "$polyunsaturated"},
            ...
            "high_sodium_count": {"$sum": {"$cond": [{"$gt": ["$sodium_per_serving", 600]}, 1, 0]}},
    ...
    },
    {
        "$project": {
            ...
            "protein_fat_ratio": {"$round": [{"$divide": ["$avg_protein", "$avg_fat"]}, 4]},
            "protein_per_100kcal": {
                "$round": [
                ...
                "$round": [
                    {
                        "$cond": [
                            {"$gt": ["$avg_fat", 0]},
                            {"$multiply": [{"$divide": ["$avg_unsaturated_fat", "$avg_fat"]}, 100]},
                            0
                        ]
                    },
                    2
            ...
            "macronutrient_balance": {
                "$round": [
                    {"$divide": ["$avg_protein", {"$add": ["$avg_protein", "$avg_fat", "$avg_carbs"]}]},
                    4
                ]
            },
